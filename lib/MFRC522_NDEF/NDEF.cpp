...
FOUND_MESSAGE NDEF::decode_message(uint8_t * msg) {
    ...
    switch ((int)tnf) {
        ...
        case 2: {
            ...
            char mimetype [typeLength];
            char payload [NDEF_BUFFER_SIZE];

            memcpy(mimetype, msg+offset, typeLength);
            *(mimetype + typeLength) = 0x00;
            m.format = (char *)(uint8_t*)mimetype;
            Serial.print("mimetype: ");
            Serial.print(m.format);
            Serial.println();

            memcpy(payload, msg+offset+typeLength, payloadLength);
            *(payload + payloadLength) = 0x00;
            m.payload = (uint8_t*)payload;
            Serial.print("data: ");
            Serial.print((char*)m.payload);
            Serial.println();
            break;
        }
        default: {
